...
(function ($) {
  var $notice_container = $(".prismal-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.prismal_admin) {
    return;
  }

  var _window$prismal_admin = window.prismal_admin,
      builderStatusData = _window$prismal_admin.builderStatusData,
      getStartedData = _window$prismal_admin.getStartedData;
  ...
  function toggleProcessing(value) {
    $(window).on("beforeunload.prismal-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".prismal-admin-big-notice").addClass("processing");
      $(".prismal-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".prismal-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.prismal-admin-big-notice");
    });
  }
  ...
  var $notice_root = $notice_container.closest(".prismal-admin-big-notice");
  var $custom_close_button = $notice_root.find(".prismal-notice-dont-show-container");

  if ($custom_close_button.length) {
    $custom_close_button.on("click", function () {
      disableNotice();
      $notice_container.closest(".prismal-admin-big-notice").find("button.notice-dismiss").click();
    });
  } else {
  ...
  window.prismal_admin.showOverlay = showOverlay;
})(jQuery);
})();
