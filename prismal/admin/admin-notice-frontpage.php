...
wp_localize_script(
	...
	array(
		...
		'builderStatusData' => array(
			'status'                          => prismal_theme()->getPluginsManager()->getPluginState( prismal_get_builder_plugin_slug() ),
			'install_url'                     => prismal_theme()->getPluginsManager()->getInstallLink( prismal_get_builder_plugin_slug() ),
			'activate_url'                    => prismal_theme()->getPluginsManager()->getActivationLink( prismal_get_builder_plugin_slug() ),
			'slug'                            => prismal_get_builder_plugin_slug(),
...
<div class="prismal-admin-big-notice--container">
	<div class="logo-holder">
        <h2><?php echo sprintf(__("Choose one of the %s predefined designs to start with:", 'prismal'), ucfirst(get_stylesheet())); ?></h2>
	</div>
	<div class="content-holder">
		<ul class="predefined-front-pages">
            ...
            $front_page_designs =  array(
                array(
                    'name'    => __( 'Modern', 'prismal' ),
                    'index'   => 1,
                    'preview' => 'front-page-1.jpg',
                ),

                array(
                    'name'    => __( 'Fullscreen', 'prismal' ),
                    'index'   => 2,
                    'preview' => 'front-page-2.jpg',
                ),

                array(
                    'name'    => __( 'Classic', 'prismal' ),
                    'index'   => 3,
            ...
            foreach ( $front_page_designs as $prismal_design_index => $design ) : ?>
				<?php
					$prismal_design_selected = $prismal_design_index === 0 ? 'selected' : '';
				...
				<li data-index="<?php echo esc_attr( $design['index'] ); ?>"
					class="<?php echo $prismal_design_selected; ?>">
                    ...
                    <div class="predefined-front-page-card">
						...
						<div class="front-page-design-wrapper">
							<div class="design-preview-image"
								 style="background-image: url(<?php echo esc_attr( prismal_theme()->getAssetsManager()->getBaseURL() . "/images/{$preview_image_name}" ); ?>)"
							></div>
						</div>
