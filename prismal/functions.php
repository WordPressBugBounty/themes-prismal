...
function prismal_register_components( $components ) {
...
Hooks::prefixed_add_filter( 'components', 'prismal_register_components', 20 );
Theme::load(
	array(
 ...
 * @return Theme
 */
function prismal_theme() {
	return Theme::getInstance();
}

function prismal_assets() {
	return prismal_theme()->getAssetsManager();
}
...
add_action('after_setup_theme', function() {
    prismal_theme()->register_menus(
        array(
            'header-menu' => esc_html__( 'Header Menu', 'prismal' ),
            'footer-menu' => esc_html__( 'Footer Menu', 'prismal' ),
        )
    );
}, 1);
if ( ! apply_filters( 'kubio_is_enabled', false ) ) {
	prismal_assets()
		...
		->registerStylesheet( 'prismal-theme', '/theme/theme.css' );
	prismal_assets()->loadLocalGoogleFonts();
	//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
	//		prismal_assets()->addGoogleFont( $family, $variants );
	//	}

	add_action( 'wp_enqueue_scripts', 'prismal_print_color_scheme', 0 );
} else {
	prismal_assets()
		->registerStyle( 'prismal-theme', prismal_assets()->getBaseURL() . '/theme/fse-base-style.css' );
	Hooks::prefixed_add_filter( 'skip_google_fonts', '__return_true' );
}

function prismal_theme_add_woocommerce_support() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action( 'after_setup_theme', 'prismal_theme_add_woocommerce_support' );

add_filter( 'kubio/activation/after_activation_redirect_url', 'prismal_after_activation_redirect_url' );
function prismal_after_activation_redirect_url( $url ) {
...
function prismal_get_builder_plugin_slug() {
...
function prismal_theme_plugins( $plugins ) {

	$plugin_slug = prismal_get_builder_plugin_slug();

	$plugins[ $plugin_slug ] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __( 'Kubio PRO', 'prismal' ) : __( 'Kubio', 'prismal' ),
...
Hooks::prefixed_add_filter( 'theme_plugins', 'prismal_theme_plugins' );
...
Hooks::prefixed_add_action(
	...
	function ( $slug ) {
		...
		if ( $slug === prismal_get_builder_plugin_slug() ) {
                ...
                <div class="kubio-admin-row get-started-imported kubio-notice notice notice-success">
                    <div>
                        <p class="imported-title">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported!', 'prismal' ),
									prismal_theme()->getName()
								)
							);
							?>
                        </p>
                        <p class="imported-subtitle">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported! You can take a look at your new design or start editing it', 'prismal' ),
									prismal_theme()->getName()
								)
							);
							?>
                    ...
                    <div class="button imported-view-site-button">
                        <a href="<?php echo esc_url( site_url() ); ?>">
							<?php echo esc_html( __( 'View site', 'prismal' ) ); ?>
                        </a>
...
function prismal_try_online_url() {
	return 'https://kubiobuilder.com/go/try-theme/' . get_template();
}

add_filter( 'kubio/editor-try-online/url', 'prismal_try_online_url', 10 );

function prismal_render_header_style() {

	?>
    <base target="_top">
	<?php

	prismal_theme()->get( 'css' )->render();
}
add_action( 'wp_head', 'prismal_render_header_style', 100 );
